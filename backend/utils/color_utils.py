...
def hex_to_color_name(hex_color):
    ...
    Uses HSV-based hue detection for dark colors to avoid
    mapping all dark-but-saturated colors to 'Black'.
    """
    ...
    # ---- HSV-BASED DARK COLOR HANDLING ----
    # Convert to HSV to detect hue in dark colors
    max_c = max(r, g, b)
    min_c = min(r, g, b)
    v = max_c  # Value (brightness) 0-255
    delta = max_c - min_c
    
    # Saturation (0.0 - 1.0)
    s = (delta / max_c) if max_c > 0 else 0.0
    
    # For dark colors (V < 115): use hue to determine color family
    # Only if the color has real saturation (not grey/black)
    if v < 115 and s > 0.10:
        # Calculate hue (0-360)
        if delta == 0:
            h = 0
        ...
        print(f"[ColorUtils] Dark/Muted Color Detected: Hex={hex_color} HSV=({h:.1f}, {s:.2f}, {v:.1f})")

        # Map hue ranges to dark color names
        # These are hue-based dark variants that exist in fashion DBs
        if 0 <= h < 20 or h >= 330:
            ...
            return "Navy Blue"    # Dark blue
        elif 260 <= h < 290:
            return "Purple"       # Dark purple
        elif 290 <= h < 330:
            return "Wine"         # Dark magenta/pink
    
    # For truly dark AND achromatic colors → Black
    if v < 45 and s < 0.10:
        return "Black"
    
    # ---- STANDARD RGB DISTANCE MATCHING ----
    best_match = "Multi"
